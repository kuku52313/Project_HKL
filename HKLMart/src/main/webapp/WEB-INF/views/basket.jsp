...
<c:choose> <c:when test="${fn:length(orderBasketList) == 0}">
    <div class="frame">
        <h2>주문상품 정보</h2>
        <h3>장바구니가 비었습니다.</h3>
    </div>
                        ...
                        <div class="order_item">
                            <dl class="basket-header">
                                <dt style="text-align: center">
                                    선택 <input type="hidden" id="checkAll" name="checkAll" value="0">
                                </dt>
                                <dt></dt>
                            ...
                            <c:forEach items="${orderBasketList}" var="list" varStatus="status" begin="0" end="${fn:length(orderBasketList)-1}">
                                ...
                                <input type="hidden" id="productCode${status.index}" name="orderList[${status.index}].productCode" value="${list.productCode}" /> <input type="hidden" id="paymentIndex" value="${fn:length(orderBasketList)}" />
                                <dl class="order_item">
                                    <dd>
                                    ...
                                    <dd>
                                        ...
                                        SIZE <input id="show-select-size${status.index}" name="orderList[${status.index}].productSize" class="form-control" type="text" value="" readonly placeholder="사이즈를 선택하세요" style="width: 30%; background-color: white; display: inline" />
                                        ...
                                        <br>
                                        수량 <input id="product-number${status.index}" name="orderList[${status.index}].productNumber" class="form-control" type="number" min="0" max="0" step="1" placeholder="상품개수" onclick="selectNumber(${status.index}), checkSum(chkForm)" style="width: 30%; display: inline" />
                                    </span>
                                    </dd>
                                    <dd>
                                        <span id="show-product-price${status.index}" style="display: inline; color: black; font-weight: bold"></span>
                                        원
                                    </dd>
